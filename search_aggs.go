...
type Aggregations map[string]jsoniter.RawMessage
...
func (a *AggregationValueMetric) UnmarshalJSON(data []byte) error {
	var aggs map[string]jsoniter.RawMessage
...
func (a *AggregationScriptedMetric) UnmarshalJSON(data []byte) error {
	...
	if v, ok := aggs["value"]; ok && v != nil {
		decoder := jsoniter.NewDecoder(bytes.NewReader(v))
		decoder.UseNumber()
